...
class DBUtils {
    ...
    async insert(table, values, columnsName = []) {
        const placeHolder = values.map((_, i) => `$${i + 1}`).join(', ');

        const columns = columnsName.length > 0 ? `(${columnsName.join(', ')})` : '';

        const queryText = `INSERT INTO ${table} ${columns} VALUES (${placeHolder}) RETURNING id`;
